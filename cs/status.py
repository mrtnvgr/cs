...
def gen(cs_name, light, cs_path, cs_type="colorscheme"):
    # Set status path
    path = os.path.join(util.paths["cache"], "status.json")

    # Status default dict
    status = {"colorscheme": {"name": cs_name, "light": light, "path": cs_path}}
    ...
    status["colorscheme"]["type"] = cs_type

    # If colorscheme is wallpaper generated
    if cs_type == "wallpaper":
